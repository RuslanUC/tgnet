...
from tgnet import Headers, Datacenter, IP, AuthCredentials, Salt, NativeByteBuffer, TGAndroidSession
...
def test_headers():
    ...
    buf = NativeByteBuffer(to_serialize)
    headers.serialize(buf)
    assert to_serialize.getvalue() == (b'\xc0\x17\x00\x00\x05\x00\x00\x007\x97y\xbc7\x97y\xbc\x05en-us\x00\x00\xb5ur'
    ...
    buf = NativeByteBuffer(to_deserialize)
...
def test_session():
    h = test_headers()
    sess = TGAndroidSession(h, [test_datacenter()])
    to_serialize = BytesIO()
    buf = NativeByteBuffer(to_serialize)
    sess.serialize(buf)
    ...
    to_deserialize = BytesIO(to_serialize.getvalue())
    buf = NativeByteBuffer(to_deserialize)
    deserialized_sess = TGAndroidSession.deserialize(buf)
